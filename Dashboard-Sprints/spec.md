...
- **FR-002b**: El sistema DEBE ejecutarse por completo en el ordenador local del usuario abriendo un único fichero, sin proceso de instalación, servidor, ni dependencias externas que instalar, y DEBE funcionar completamente offline (sin conexión a internet), incluidos los gráficos. El flujo debe ser: abrir el dashboard → subir el/los CSV → ver el dashboard interactivo actualizado con esos datos.
...
- **FR-006**: El sistema DEBE conservar los datos de los sprints ya cargados entre visitas al dashboard, sin necesidad de volver a subirlos.
...
- El dashboard es de uso individual/local para el Scrum Master en esta primera versión; compartirlo con más personas del equipo queda fuera de alcance de esta spec.
- **Sin autenticación**: el dashboard NO requiere login ni ningún tipo de autenticación de usuario; se abre y se usa directamente. Esto es coherente con ser un fichero local de un único usuario, sin backend ni almacenamiento compartido.
- **Distribución y ejecución**: el dashboard DEBE poder ejecutarse en el ordenador local de cualquier persona simplemente abriéndolo, sin necesidad de instalar nada (sin servidor, sin dependencias, sin proceso de instalación) y DEBE funcionar completamente sin conexión a internet, incluida la generación de los gráficos (la librería de gráficos debe ir incluida en el propio fichero, no cargada desde un CDN externo). El flujo de uso es: abrir el dashboard → subir el/los CSV → ver el dashboard interactivo.
